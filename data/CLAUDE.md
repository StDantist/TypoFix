...
- **`dicts/{lang}.freq.fst`** (gitignored) — **частотний словник** `fst::Map(слово
  → count)` з розмовного корпусу OpenSubtitles (`fetch_freq.py`, MIT). Дає core
  ГРАДУЙОВАНИЙ сигнал (на відміну від бінарного членства `Dictionary`): часте слово
  (`the`,`ну`) → не чіпати; рідкісне/відсутнє (`ye`,`lox`) → можна перемкнути.
  Інтерфейс (typofix-data): `build_freq_map`/`load_freq_map_file`/`freq_lookup` →
  `freq(word)->Option<u64>`. **None ≠ «не слово»**: рідкісні флексії довгого хвоста
  VESUM не мають freq-запису, але Є членами `Dictionary` → core дає їм baseline-бонус,
  freq лише ДОДАЄ зважування. Тип-обгортку для `LanguageProfile` визначає core (Den).
  Пайплайн частот: `fetch_freq.py` → `corpora/freq/{lang}.freq.txt` → `train_models`.
  Короткі укр. (≤3) у `uk.fst` тепер проходять подвійний гейт `freq≥200 ∩ VESUM`
  (`fetch_dict_uk.py` читає `uk.freq.txt`) — часті розмовні (`що/щоб/ну/ха/то/ще`)
  всередині, шум (`ші/ру/ге`) ні. Тому ПОРЯДОК: `fetch_freq.py` ПЕРЕД `fetch_dict_uk.py`.
- **`dicts/iso4217.txt`** (committed) — активні коди валют ISO 4217 (один код на
  рядок, UPPERCASE, `#`-коментарі). Дані для **Forex-перемикання** у core: токен
  `^[A-Z]{6}$`, де `[0:3] ∈ ISO ∩ [3:6] ∈ ISO` (EURUSD, GBPUSD, XAUUSD…) у НЕ-
  поточній розкладці → **форс-перемикання на латиницю** (пара, набрана у випадково
  ввімкненій укр. розкладці, стає `EURUSD`); коректно набрана латинська пара —
  недоторкана. Системно: множина кодів + патерн, не хардкод пар. **Метали**
  (XAU/XAG/XPT/XPD) лишено (реальні Forex-інструменти); **виключено** тест/службові
  (XXX, XTS), bond units (XBA-XBD), fund-коди, supranational accounting (XUA, XSU)
  — зменшує хибні veto на 6-літерних UPPERCASE-токенах. Loader (typofix-data):
  `parse_iso4217(&str)`/`embedded_iso4217()`/`load_iso4217(&Path)->HashSet<String>`
  (UPPERCASE-коди; нема файлу → вбудований перелік). Хелпер `is_currency_pair(&set,
  token)->bool` — суто перевірка членства; **власне рішення у `step` робить core (Den)**.
- **`dicts/user.txt`** (committed-шаблон) — **особистий словник: ПОЗИТИВНЕ
  розпізнавання** (НЕ veto). Слова, які користувач хоче, щоб апка визнавала як
  валідні й ПЕРЕМИКАЛА на них: жаргон/терміни поза стандартним словником, надто
  рідкісні для авто-словника (напр. `лох`). Core тримає їх як додаткову dict-
  членність (`WordRules.recognized`) → той самий baseline dict_bonus, і вони
  входять у `best_is_dict`. Один термін на рядок, `#`-коментарі, порожні рядки
  ігноруються, краї trim, регістр не важливий (матчинг у core регістронезалежний).
  Власник дописує рядки сам; зміни підхоплюються на reload, без перезбірки. Loader:
  `parse_user_words(&str)`/`load_user_words(&Path)->Vec<String>` (нема файлу →
  порожній Vec).

...
fn non_dict_word_ending_in_punct_letter_stays_unfixed_step_b() {
    // ЗАГЛУШКА/НОТАТКА для кроку (б): `лох` закінчується на х(=`[`) → "kj[", але
    // `лох` ВІДСУТНІЙ у словнику (тут навмисно не додано; у проді — фільтр
    ...
    let (text, actions) = type_in_en_expect("лох", &[]);
    assert_ne!(
        text, "лох ",
        "крок (а) свідомо НЕ ловить не-словникове `лох`"
    );
    assert!(
        !actions
            .iter()

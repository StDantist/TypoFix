...
- «Скинути до стандартних»: після підтвердження модалки параметри повертаються до
  дефолтів (тоггл `fix_case` знову увімкнено, мова `uk-en`), а додане слово-виняток
  ЛИШАЄТЬСЯ (дані користувача не втрачено). Тест прибирає тестове слово за собою.

...
//! ДЕШЕВИЙ нативний перерахунок із власним message-pump.
//!
//! ## 🔴 БЕЗ UIA/COM
//! Перерахунок ([`crate::secure::recompute`]) робить ЛИШЕ нативну перевірку
//! (`GetWindowLongPtrW` біт `ES_PASSWORD` + `EM_GETPASSWORDCHAR` на edit-класи) —
//! без UIA/COM, бо виклик UIA вмикав a11y-дерево цільової IDE й лагав її в ЇЇ
//! процесі (репро власника: 30–40 с). Тут жодного `CoInitialize`/`GetFocusedElement`.
//!
//! ## Чому все одно окремий потік
//! LL-hook потік (`crate::hook`) лишається абсолютно дешевим — `is_secure_field` на
//! гарячому шляху лише читає атомік `secure::CACHE`. Навіть мікросекундний нативний
//! перерахунок ведемо тут, поза потоком вводу, і коалесимо дебаунсом.
//!
//! ## Дебаунс (коалесинг шторму)
//! IDE сипле фокус-події пачками. Кожна подія НЕ запускає перерахунок одразу — вона
//! лише (пере)зводить короткий таймер ([`FOCUS_DEBOUNCE_MS`]); перерахунок іде ОДИН
//! раз після «осідання» фокуса (нативна перевірка дешева, але дебаунс зайвим не буде).
...
/// Тіло потоку: WinEvent-хуки фокуса → pump із дебаунс-перерахунком (нативним).
fn secure_thread_main(tid_out: Arc<AtomicU32>) {
    // Скидаємо кеш (нова сесія) і рахуємо для поточного фокуса одразу. Жодного COM —
    // перерахунок суто нативний.

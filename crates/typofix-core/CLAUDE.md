...
- **⚠️ НИЖНЯ МЕЖА — ЛІТЕРАЛ `len >= 2`, НЕ `cfg.min_switch_len` (ДВА репро!).**
  ...
  - **Репро A (precision, кома):** клавіша `,`(0x33) у EN — пунктуація, у UK — літера
    «б» (у whitelist + `uk.fst`). Без межі самотня кома форсилась на «б». Floor `2`
    блокує len=1 → кома лишається комою. Самотні літери («б»/«о»/«я») практично
    ніколи не є самостійним словом, вартим перемикання.

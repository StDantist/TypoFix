...
- **`dicts/iso4217.txt`** (committed) — активні коди валют ISO 4217 (один код на
  рядок, UPPERCASE, `#`-коментарі). Дані для **veto валютних пар** у core: токен
  `^[A-Z]{6}$`, де `[0:3] ∈ ISO ∩ [3:6] ∈ ISO` (EURUSD, GBPUSD, XAUUSD…) → НЕ
  перемикати. Системно: множина кодів + патерн, не хардкод пар. **Метали**
  ...
  token)->bool` — суто перевірка членства; **власне veto в `step` робить core (Den)**.
- **`dicts/user.txt`** (committed-шаблон) — **особистий словник «ніколи не чіпати»**
  (veto): жаргон/нікнейми/бренди/рідкісні слова, надто нечасті для авто-словника
  (напр. `лох`). Один термін на рядок, `#`-коментарі, порожні рядки ігноруються,
  краї trim, регістр не важливий (матчинг у core регістронезалежний). Власник
  дописує рядки сам; зміни підхоплюються на reload, без перезбірки. Loader:
  `parse_user_words(&str)`/`load_user_words(&Path)->Vec<String>` (нема файлу →
  порожній Vec). **Core тримає як veto-набір; нормалізацію регістру робить core.**

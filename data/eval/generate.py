...
# 1-літерні укр. СЛОВА, що ПЕРЕМИКАЮТЬСЯ (куроване рішення власника): реальні
# однолітерні укр. слова у,а,в,з,і,о,є,я,с, чиї en-джерела (e,f,d,p,s,j,',z,c) —
# беззмістовні окремо. switch=true, typed=en, intended=uk. Замок precision у ядрі —
# 1-літерний whitelist (`uk.short.txt`/`single_letter_service`): реальні англ.
# одиночні a/i/o мапляться в укр. НЕ-слова (ф/ш/щ) → не перемикаються (див.
# NEG_SHORT_EN + NEG_SHORT_SINGLE). Будь-яка зміна цього набору — синхронно з
# «ПОЛІТИКОЮ ПЕРЕМИКАННЯ» (crates/typofix-core/CLAUDE.md). Раніше ці приклади були
# негативами (надмірна блокада ВСІХ одиночних) — це й була регресія, яку чинимо.
UK_SHORT1 = ["у", "а", "в", "з", "і", "о", "є", "я", "с"]
...
NEG_SHORT_EN = [
    "a", "i", "o", "am", "as", "be", "do", "go", "he", "if", "me", "my", "ok",
    "so", "up", "us", "we", "oh",
]
...
# «x» прибрано не можна — c прибрано: `c`→`с` тепер ПЕРЕМИКАЄТЬСЯ (с ∈ whitelist),
# тож `c` пішов у POS (UK_SHORT1). Решта — uk-двійник не слово (g->п, h->р, n->т…).
NEG_TWIN = [
    "g", "h", "k", "l", "m", "n", "r", "t", "u", "w", "x",
    "qw", "zx", "fg", "kl", "vn",
]
# Одиночні символи, що НЕ перемикаються (precision-замок ПОЛІТИКИ ПЕРЕМИКАННЯ):
#  - кома `,` — клавіша-роздільник; її uk-двійник «б» НЕ в 1-літерному whitelist
#    (репро власника: кома в EN НЕ сміє ставати «б»);
#  - `q`→`й` — «й» свідомо ПОЗА курованим списком власника (у/а/в/з/і/о/є/я/с).
# Реальні англ. одиночні a/i/o — у NEG_SHORT_EN (їхні uk-двійники ф/ш/щ — не слова).
# Раніше тут були ВСІ 8 одиночних укр. (надмірна блокада) — звужено до справді
# незмикальних. switch=false, typed=en.
NEG_SHORT_SINGLE = [",", translate("й", UK2EN)]
...
def build():
    ...
    # POSITIVE: 1-літерні укр. СЛОВА (у/а/в/з/і/о/є/я/с) у en-розкладці —
    # перемикаються (куроване рішення власника; джерела e/f/d/p/s/j/'/z/c).
    for w in UK_SHORT1:
        rows.append(positive(translate(w, UK2EN), "en", "uk", "pos_short_uk_1"))
    ...
    # NEGATIVE: одиночні символи, що НЕ перемикаються (кома `,`→`б`; `q`→`й` поза
    # курованим whitelist) — precision-замок ПОЛІТИКИ ПЕРЕМИКАННЯ.

...
- **`dicts/user.txt`** (committed-шаблон) — **особистий словник: ПОЗИТИВНЕ
  ...
  рідкісні для авто-словника (напр. `вжух`). Core тримає їх як додаткову dict-
  ...
  порожній Vec).
- **`dicts/extensions.txt`** (committed) — добре відомі файлові розширення БЕЗ
  крапки (один на рядок, lowercase, `#`-коментарі). Сценарій: англ. розширення,
  набране в укр. розкладці, виходить безглуздям (`txt`→`еche`, `md`→`ьв`) — таких
  слів нема → **перемкнути на латиницю** (switch, не veto). Добір: ЛИШЕ добре
  відомі (precision > recall). Loader (typofix-data): `parse_extensions(&str)`/
  `embedded_extensions()`/`load_extensions(&Path)->HashSet<String>` (lowercase;
  нема файлу → вбудований). Хелпер `is_known_extension(&set, token)->bool`
  (lowercase membership, ігнорує провідну крапку). **Власне switch-рішення в
  `step` робить core (Den).**
  - **РИЗИКОВІ розширення (для Den — precision-гейт).** Деякі токени збігаються з
    реальними словами/токенами; членство в множині саме по собі НЕ привід
    перемикати — Den має додати гейт «не чіпати, якщо укр. читання — реальне укр.
    слово, АБО токен — валідне en-слово/частий код». Найризиковіші:
    - **EN-слова:** `doc` (укр. читання `вщс`? — ні, але «doc» = слово), `log`,
      `key`*, `bin`, `app`*, `go`, `cs`, `js`*, `ts`*, `db`, `iso`, `tar`, `bat`,
      `c`, `h`, `sh`, `gz`. (* `key`/`app` поки НЕ в списку — приклад потенційних
      доповнень; перевір перед додаванням.)
    - **Короткі (≤2 літери) — найвищий ризик колізій:** `go` `js` `ts` `rs` `py`
      `rb` `db` `sh` `cs` `c` `h` `md`. Для них укр. читання теж коротке → легко
      збігається з укр. службовими словами (`go`→`пщ`, `db`→`вис`) або en-токенами
      коду. Den: для ≤2-літерних вимагати сильнішого сигналу (напр. наявність
      крапки-роздільника `name.go`, а не голий токен).
    - **Збіг з укр. словом при укр.-читанні:** перевіряти проти uk-словника/freq
      (вже є `uk.fst`/`uk.freq.fst`) — якщо укр. читання токена частотне, не чіпати.

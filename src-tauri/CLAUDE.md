...
## Демо-харнес `examples/live_engine.rs` (жива перевірка без GUI)
Реальні моделі (`TYPOFIX_DATA_DIR`) → `WindowsPlatform` (хуки) → цикл рушія ~20 c
(Esc — раніше) → лог `[ВИПРАВЛЕНО] 'було' → 'стало' (мова X→Y)`. «Було»
реконструюється back-translate (символи перенабору → страйки в цільовій розкладці →
інтерпретація у вихідній). ⚠️ Ставить системні хуки — **лише вручну**.
- **ГОТЧА — це `examples/`, а НЕ `[[bin]]`:** навмисно, щоб `tauri build` його НЕ
  бандлив у реліз (раніше як bin він їхав «причепом» у MSI/NSIS; `required-features`
  не рятує — Tauri бандлить bin-таргети незалежно від features). `cargo build`/
  `tauri build` приклади не збирають; будуються лише на вимогу (`--example`).
- **ГОТЧА — запуск (відокремлений workspace!):** `-p typofix-app` із кореня репо НЕ
  працює. Запускати з теки `src-tauri`:
  ```powershell
  ...
  ```
  (з кореня — `cargo run --manifest-path src-tauri/Cargo.toml --example live_engine`).
Не-Windows: приклад компілюється як заглушка (друкує попередження).

...
- **⚠️ НИЖНЯ МЕЖА `len ≥ min_switch_len` (≥2) — ЗАМОК PRECISION (репро власника).**
  Дзеркало НЕ сміє форсити ОДИНОЧНИЙ токен. Корінь бага: клавіша `,`(0x33) у EN —
  пунктуація, у UK — літера «б»; «б» Є у whitelist (`uk.short.txt` 1-літерні) та в
  `uk.fst`. Без межі самотня кома, набрана в EN, форсилась на «б» — а кома в англ.
  це звичайна пунктуація. Самотні літери («б»/«о»/«я»/«і») практично ніколи не є
  самостійним словом, вартим перемикання → **жоден одиночний токен не перемикаємо**
  (precision > recall, явна вимога власника). 1-літерні рядки в `uk.short.txt` тепер
  МЕРТВІ для дзеркала (лишаються лише як dict/LM-члени у `uk.fst`) — НЕ видаляв їх,
  щоб не чіпати зону `typofix-data`; межа в core робить їх безпечними.
...
- **Калібрування (реальний eval, 383 прикл.):** precision **100%** (0 FP).
  Після введення нижньої межі: `pos_short_uk_1` (8 одиночних літер) свідомо
  **0%** (FN — одиночні не чіпаємо, прямий наслідок precision-замку коми);
  `pos_short_uk_2` **92.3%**, `pos_uk_for_en` **100%** (2-літерні службові НЕ
  просіли). Загальний recall 94.4%, F1 97.1%. **Follow-up для Bruno/оркестратора:**
  датасетна категорія `pos_short_uk_1` описує одиночні-літери-як-позитиви, що
  суперечить вимозі власника — варто рекласифікувати їх у негативи/прибрати.

...
pub trait Platform {
    ...
    /// Чи активне (фокусне) поле вводу — **секретне** (пароль)?
    ///
    /// Приватність (залізне правило №4): у секретні поля TypoFix не сміє ні
    /// буферити натиски, ні перемикати розкладку. Default = `false` (не
    /// секретне) — платформи, що не вміють це визначати, лишаються безпечно
    /// «звичайними». Windows ловить нативні Win32-поля (`ES_PASSWORD`); веб/
    /// Electron-поля — інший механізм (UI Automation), поки не покрито.
